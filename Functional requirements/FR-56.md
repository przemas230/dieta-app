...
## Uwagi (Android, 2026-08-26)
Android renderowała ten napis systemową czcionką pogrubioną
(`MaterialTheme.typography.titleLarge` + `FontWeight.Bold`) — wyraźnie
inna od web'owej niestandardowej "balonowej" czcionki (Baloo 2). Naprawione
zbindlowaniem tego samego kroju: `Baloo2[wght].ttf` (font zmienny, licencja
OFL) pobrany z oficjalnego repo Google Fonts do `res/font/
baloo2_variable.ttf`, nowa `SwipeLabelFont` (`FontVariation.weight(800)`,
`FontWeight.ExtraBold`) w `RecipeListScreen.kt`, użyta zamiast systemowej
czcionki wyłącznie na tym jednym napisie.
...
- **v6** (2026-08-26, Android): Czcionka napisu na Androidzie dopasowana do
  Web (Baloo 2, ExtraBold) — patrz sekcja "Uwagi (Android, 2026-08-26)".
  `./gradlew :app:compileDebugKotlin` przechodzi. **Nie zweryfikowane
  wizualnie na emulatorze w tej turze.**
